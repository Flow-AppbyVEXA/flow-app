...
export function AuthProvider({ children }) {
  ...
  const register = async (email, pw, businessName) => {
    const cred = await createUserWithEmailAndPassword(auth, email, pw)
    await setDoc(doc(db, 'users', cred.user.uid, 'data', 'main'), makeInitData(businessName))
    return cred
  }

  if (authLoading) return (
    <div style={{ height:'100vh', display:'flex', alignItems:'center', justifyContent:'center', background:'#0f1923', fontFamily:"'Nunito',sans-serif" }}>
